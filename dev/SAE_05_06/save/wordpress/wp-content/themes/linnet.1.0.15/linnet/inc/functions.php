...
function linnet_page_builder_components($components)
{
...
function linnet_default_components($components)
{
...
function linnet_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = linnet_page_builder_components($components);
    } else {
        $components = linnet_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'linnet_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function linnet_assets()
{
    return linnet_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    linnet_theme()->register_menus(array(
        'header-menu' => esc_html__('Header Menu', 'linnet'),
        'footer-menu' => esc_html__('Footer Menu', 'linnet'),
    ));
...
add_action('widgets_init', 'linnet_register_sidebars');
function linnet_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'linnet'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'linnet'),
            'id' 			=> 'linnet-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        linnet_assets()
            ...
            ->registerStylesheet("linnet-theme", "/theme/theme.css")
    ...
    linnet_assets()->registerTemplateStyle('linnet-theme-extras', '/theme/extras.css', 'linnet-theme');
...
//blog options
    ...
    function linnet_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'linnet_show_post_meta_setting_filter');


    function linnet_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'linnet_posts_per_row_setting_filter');

    function linnet_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'linnet_archive_post_highlight_setting_filter');


    function linnet_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'linnet_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'linnet_blog_sidebar_enabled_setting_filter' );

    function linnet_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'linnet_override_with_thumbnail_image');

    function linnet_print_archive_entry_class($class = "")
    {
        ...
        $postsPerRow = max(1, apply_filters('linnet_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('linnet_archive_entry_class', $classes);
    ...
    function linnet_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('linnet_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'linnet_get_started_info_page_tab');

    function linnet_get_started_info_page_tab($tabs)
    {
    ...
    function linnet_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'linnet_theme_plugins');


    add_filter('http_request_host_is_external', 'linnet_allow_internal_host', 10, 3);
    function linnet_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_linnet_front_set_predesign', function () {
        check_ajax_referer( 'linnet_front_set_predesign_nonce', 'nonce' );
    ...
    function linnet_override_main_row_class($classes)
    {
        return Defaults::get('templates.blog.row.layout-classes', $classes);
    }

    Hooks::prefixed_add_filter('main_row_class', 'linnet_override_main_row_class', 10, 1);
